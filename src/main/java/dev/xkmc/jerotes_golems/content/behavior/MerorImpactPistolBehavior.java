...
public class MerorImpactPistolBehavior implements ICrossbowBehavior {

	@Override
	public int chargeTime(LivingEntity user, ItemStack stack) {
		return MerorImpactPistol.getChargeDuration(stack);
	}
	...
	public List<ItemStack> getLoadedProjectile(LivingEntity user, ItemStack stack) {
		return MerorImpactPistol.getBullet(stack) > 0 ? List.of(JerotesVillageItems.MEROR_BULLET.get().getDefaultInstance()) : List.of();
	}
	...
	public boolean tryCharge(ProjectileWeaponUser user, ItemStack stack) {
		...
		MerorImpactPistol.setBullet(stack, 6);
		return true;
	}
	...
	public int performRangedAttack(CrossbowUseContext user, ItemStack stack, InteractionHand hand) {
		...
		user.aim(pos, 3, 0, 0).shoot(shoot, 0);
